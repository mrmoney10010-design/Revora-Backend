...
class MockOfferingRepo {
  ...
  async listPublic(opts: any) {
    ...
    // Stable sort implementation: created_at DESC, id ASC
    out = out.sort((a,b) => {
...
describe('offerings catalog routes', () => {
  ...
  const rows = [
    { id:'11111111-1111-4111-8111-111111111111', title:'A', issuer_id:'s1', status:'active', amount:'100.00', created_at:sameTime, revenue_share_bps:500, private_note: 'issuer-only' },
    { id:'33333333-3333-4333-8333-333333333333', title:'C', issuer_id:'s3', status:'active', amount:'300.00', created_at:sameTime, revenue_share_bps:700 },
    { id:'22222222-2222-4222-8222-222222222222', title:'B', issuer_id:'s2', status:'active', amount:'200.00', created_at:sameTime, revenue_share_bps:300 },
  ];

  it('covers public catalog stable ordering and detail visibility rules', async () => {
    const repo = new MockOfferingRepo(rows);
    const handlers = createPublicHandlers(repo as any);

    // List active - should be ordered by created_at DESC (same), then id ASC
    ...
    assert(out1.jsonData.offerings.length === 3);
    assert(out1.jsonData.offerings[0].id === '11111111-1111-4111-8111-111111111111');
    assert(out1.jsonData.offerings[1].id === '22222222-2222-4222-8222-222222222222');
    assert(out1.jsonData.offerings[2].id === '33333333-3333-4333-8333-333333333333');

    // Pagination limit
    const req2 = makeReq({ status: 'active', limit: '2', offset: '1' });
    const res2 = makeRes();
    await handlers.listCatalog(req2, res2, (e:any)=>{ throw e });
    const out2 = res2._get();
    assert(out2.jsonData.offerings.length === 2);
    assert(out2.jsonData.offerings[0].id === '22222222-2222-4222-8222-222222222222');
    ...
    assert(err5.statusCode === 404);

    // Invalid id format
    const req6 = makeReq({}, { id: 'not-a-uuid' });
    const res6 = makeRes();
    let err6: any;
    await handlers.getOfferingById(req6, res6, (e:any)=>{ err6 = e });
    assert(err6.code === 'BAD_REQUEST');
    assert(err6.statusCode === 400);
  });

  it('validates limit and offset correctly', async () => {
    const repo = new MockOfferingRepo(rows);
    const handlers = createPublicHandlers(repo as any);

    // Invalid limit
    const reqL = makeReq({ limit: '-1' });
    const resL = makeRes();
    let errL: any;
    await handlers.listCatalog(reqL, resL, (e:any)=>{ errL = e });
    assert(errL.code === 'BAD_REQUEST');

    const reqL2 = makeReq({ limit: 'invalid' });
    const resL2 = makeRes();
    let errL2: any;
    await handlers.listCatalog(reqL2, resL2, (e:any)=>{ errL2 = e });
    assert(errL2.code === 'BAD_REQUEST');

    // Invalid offset
    const reqO = makeReq({ offset: '-5' });
    const resO = makeRes();
    let errO: any;
    await handlers.listCatalog(reqO, resO, (e:any)=>{ errO = e });
    assert(errO.code === 'BAD_REQUEST');
  });

  it('handles unimplemented listPublic gracefully', async () => {
    const repo = {
      // missing listPublic
    };
    const handlers = createPublicHandlers(repo as any);
    ...
    const res = makeRes();
    let err: any;
    await handlers.listCatalog(req, res, (e:any)=>{ err = e });
  ...
  it('handles database errors gracefully', async () => {
    const errorRepo = {
      listPublic: async () => { throw new Error('DB timeout'); },
      getById: async () => { throw new Error('DB connection closed'); }
    };
    const handlers = createPublicHandlers(errorRepo as any);
    
    const req1 = makeReq({});
    const res1 = makeRes();
    let err1: any;
    await handlers.listCatalog(req1, res1, (e:any)=>{ err1 = e });
    assert(err1.message === 'DB timeout');

    const req2 = makeReq({}, { id: '11111111-1111-4111-8111-111111111111' });
    const res2 = makeRes();
    let err2: any;
    await handlers.getOfferingById(req2, res2, (e:any)=>{ err2 = e });
